export default async function handler(req, res) {
  ...
  // V2 API endpoint - chain 8453 = Base mainnet
  ...
  async function bscan(params) {
    const qs = new URLSearchParams({ ...params, chainid: CHAIN, apikey: KEY }).toString();
  ...
  try {
    ...
    const hasBasename  = [CONTRACTS.BASENAME_REGISTRAR, CONTRACTS.BASENAME_RESOLVER, CONTRACTS.BASENAME_REG2].some(c => allToAddrs.has(c));
    const hasAerodrome = [CONTRACTS.AERODROME_ROUTER, CONTRACTS.AERODROME_ROUTER2].some(c => allToAddrs.has(c));
    const hasUniswap   = [CONTRACTS.UNISWAP_V3_ROUTER, CONTRACTS.UNISWAP_UNIVERSAL].some(c => allToAddrs.has(c));
    const hasNFT       = [CONTRACTS.ZORA_1155, CONTRACTS.ZORA_FACTORY, CONTRACTS.OPENSEA_SEAPORT].some(c => allToAddrs.has(c)) || nfts.length > 0;
    const hasZora      = [CONTRACTS.ZORA_1155, CONTRACTS.ZORA_FACTORY].some(c => allToAddrs.has(c));
    const hasLending   = [CONTRACTS.MOONWELL, CONTRACTS.MOONWELL_USDC, CONTRACTS.COMPOUND_USDC].some(c => allToAddrs.has(c));
    ...
    return res.status(200).json({
      address: addr, score, risk, flags,
      tier: { level:tier, label:tierLabel, estMin, estMax, color:tierColor },
      checklist: { items:checklist, done:checklistDone, total:checklist.length, points:checklistPoints, maxPoints:checklistMaxPoints },
      data: {
